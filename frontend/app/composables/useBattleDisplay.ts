...
export function resistColor(val: string) { return RESIST_COLORS[val] ?? '#555' }

/** CSS colour for a turn-state badge. */
export function turnColor(val: string)   { return TURNSTATE_COLORS[val] ?? '#888' }
...
export function sgPct(unit: any)  {
  return unit.maxStaggerGauge > 0 ? Math.min(100, unit.staggerGauge / unit.maxStaggerGauge * 100) : 0
}
...
export function coinDots(unit: any): string {
  ...
  if (!coins) return ''
  return '●'.repeat(coins.positive) + '○'.repeat(coins.negative)
...
export const MASS_RANGES = new Set([
  'FarArea', 'FarAreaEach',
])

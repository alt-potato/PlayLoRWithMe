# multi-deck-key-page-editor Specification

## Purpose

Defines the deck editor's behavior for multi-deck key pages — key pages whose `BookModel.IsMultiDeck()` returns `true` and which therefore expose four `DeckModel` slots (one per stance) rather than a single deck. The editor surfaces every slot through a tab strip, scopes all add/remove actions and pending state to the active tab, and enforces the engine's per-deck-slot copy limits. Single-deck key pages render the editor exactly as before this change. The server transparently switches `BookModel`'s active deck index across the mutation and restores it after each action so battle-time UI is unaffected.

...
### Requirement: Mock fixture catalog SHALL cover combinatoric and edge-case UI surfaces

The fixture catalog MUST include at least the following named fixtures, each a static full `GameState` snapshot:

- `battle-sampler` — battle scene with allies and enemies. The ally hand MUST include at least one card of every `CardRange` value supported by the UI (`Near`, `Far`, `Instance`, `Special`, `FarArea`, `FarAreaEach`) and at least one card of each rarity (`Common`, `Uncommon`, `Rare`, `Unique`). It MUST include at least one card with a non-empty `subTargets[]`, at least one card with an overflow-prone `abilityDesc` (long enough to trigger detail-pane vertical scrolling), at least one broken unit (`turnState === "BREAK"`), and at least one dead unit.
- `main-librarian` — main scene with `floors[]` populated. MUST include at least one locked librarian, at least one unlocked librarian, at least one librarian without a key page equipped, and a non-empty `availableKeyPages[]` / `availableCards[]`.
- `battle-setting` — main scene with `uiPhase === "BattleSetting"` so `BattleSettingView` is the active scene router branch.
- `emotion-upgrade` — battle scene with `abnormalitySelection` populated (≥3 choices, at least one with `targetType === "Ally"`) so `EmotionUpgradePicker` mounts.

#### Scenario: battle-sampler covers all card ranges

- **WHEN** the `battle-sampler` fixture is loaded
- **THEN** the resolved `gameState.allies[0].hand` contains cards whose `range` values collectively cover the set `{Near, Far, Instance, Special, FarArea, FarAreaEach}`

#### Scenario: emotion-upgrade triggers picker

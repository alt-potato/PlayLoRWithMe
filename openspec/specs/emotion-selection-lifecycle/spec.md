# emotion-selection-lifecycle Specification

## Purpose

Defines the lifecycle and validity contract for the mod's emotion-card selection state — both the abnormality branch (`AbnormalitySelectionState`) and the EGO branch (`EgoSelectionState`). Selection state can otherwise outlive the battle it belongs to: if a player leaves a reception without committing a pick, a stale picker can carry over into the next reception, and a stale or replayed `selectAbnormality` / `selectEgo` action can target a floor that is no longer live. This spec requires that selection state be cleared on the scene transitions that leave or re-enter a battle, and that pick actions be honored only against the live current floor.

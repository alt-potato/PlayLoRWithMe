# abnormality-select-dismissal Specification

## Purpose

Defines the post-commit cleanup contract for the mod's `selectAbnormality` action handler. Library of Ruina's `LevelUpUI` opens with `LevelUpUI.Init` playing a looping ambience sample (`EffectSoundType.ABNORMALITY_SELECT_LOOP` into `_loopSound`) and ducking the BGM to 0.5×; in-game, those side effects are reversed by `OnSelectPassive` (or `OnSelectHide`) on the player's card click. The mod short-circuits the in-game UI for remote picks, and so must explicitly mirror the same audio-cleanup and canvas-dismissal end-state — without leaving the UI in a state-transition that would block subsequent emotion-level prompts.

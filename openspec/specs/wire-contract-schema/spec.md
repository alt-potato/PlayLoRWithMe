...
The frontend MUST cache `theme.factionDieColors.ally` and `theme.factionDieColors.enemy` as faction die CSS custom properties on the document root when present (the rendered die background and numeral are derived from each tint — see the battle-die-state capability). Absent values leave the existing CSS-default fallback values in place.
...
- **AND** the frontend writes the faction tints to the corresponding CSS vars on the document root

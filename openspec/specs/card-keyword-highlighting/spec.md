...
- **WHEN** a card has `abilityDesc === "[On Use] Gain 1 Haste."` and its detail is shown in either the hand-card detail pane or the `CardDetail` modal
- **THEN** the rendered text reads `On Use Gain 1 Haste.`
- **AND** `On Use` is rendered in the bright-gold highlight colour

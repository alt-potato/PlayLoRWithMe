# theme-handshake Specification

## Purpose
Some battle visuals (currently the per-faction speed-die tints) are defined by
the game's runtime prefabs rather than known ahead of time. This capability
defines how the mod samples those constants from Unity via reflection and ships
them to clients once on the `hello` payload (with a late-probe retry path), and
how clients tolerate their absence by falling back to hardcoded defaults.
...
- **AND** the frontend caches the two hex strings as faction die CSS vars on `document.documentElement.style` (the rendered background/numeral are derived from them — see the battle-die-state capability)
...
- **AND** the frontend's CSS reads the faction die vars' hardcoded defaults and the UI renders with them

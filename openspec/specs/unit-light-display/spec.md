# unit-light-display Specification

## Purpose

Defines how each battle unit's light pool (current / max / reserved) is carried across the wire and rendered in the unit-card header — uniformly for allies and enemies. The engine's `BattleUnitModel.PlayPoint` / `MaxPlayPoint` / `ReservedPlayPoint` triple exists on every battle unit regardless of faction, so the wire shape and the UI surface them on every unit and let the pip row's own `max > 0` guard hide the row for units (the common enemy case) with no pool.

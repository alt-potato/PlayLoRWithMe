# ego-card-selection Specification

## Purpose

Covers the end-to-end EGO-page emotion-level pick flow surfaced by `LevelUpUI.InitEgo`: the mod-side Harmony hooks that expose the open/commit lifecycle, the `egoSelection` wire payload that carries battle-card metadata (cost, dice, range, rarity, Sephirah, description), the frontend overlay rendered through the existing emotion-upgrade picker chrome, the `selectEgo` action handler that commits a remote pick, and the dismissal cleanup that hides the in-game `LevelUpUI`. The flow runs in parallel with the abnormality-selection flow it is modelled on, but uses battle-card data rather than abnormality data and skips the ally-target step entirely.

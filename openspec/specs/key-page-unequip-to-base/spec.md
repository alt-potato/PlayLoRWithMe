# key-page-unequip-to-base Specification

## Purpose

Defines how each librarian's immutable origin/base key page (`UnitDataModel.defaultBook` in the engine) is exposed across the wire and how the librarian editor lets users return to it. The base page is mechanically present at all times — the engine's `bookItem` getter falls back to it when `_bookItem` is null — but is not surfaced as a separate selectable tile. Instead, the action button in `KeyPageTab` doubles as the unequip affordance: selecting the currently-equipped inventory tile flips the button to a destructive-styled "Unequip", and pressing it dispatches the new `unequipKeyPage` action which calls `UnitDataModel.EquipBook(null)` server-side.
